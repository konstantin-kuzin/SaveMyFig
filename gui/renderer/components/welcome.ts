...
export function initializeWelcomeTab(): void {
  ...
  if (installBtn && nodeStatus) {
    ...
    window.electronAPI.checkNodeJS().then((result: any) => {
      if (result.installed) {
 ...
 if (npmStatus) {
    ...
    window.electronAPI.checkNpm().then((result: any) => {
      if (result.installed) {
    ...
    installBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (result.success) {
          ...
          setTimeout(() => {
            const configLink = document.querySelector('.nav-link[data-tab="config"]') as HTMLElement;
