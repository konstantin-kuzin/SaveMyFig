...
├── start.command             # Унифицированный лаунчер: portable Node.js 24.11.1 при отсутствии npm, npm install (root+gui), playwright install, запуск GUI
...
- Пользовательский поток: запуск через единый `start.command`, который при первом старте ставит portable Node.js 24.11.1 (если npm отсутствует), устанавливает зависимости (root+gui, Playwright) и открывает GUI; при повторных запусках пропускает установку, если lockfile не менялся.
...
### 9.2 GUI
Рекомендуемый путь для пользователей без Node/npm: запустить `start.command` в корне. Скрипт скачает portable Node.js 24.11.1 при отсутствии npm, установит зависимости в корне и в `gui/`, подтянет Playwright и откроет GUI. Повторные запуски пропускают установку, если lockfile не менялся.

Для ручного запуска из терминала (dev-режим):
```bash
cd gui
npm install
...
4. **Installation tab** не ставит Node.js внутри GUI — установка происходит только через внешний `start.command` (portable Node 24.11.1 для macOS).
...
9. **Упаковка** — нет `.app/.dmg` или отдельного лаунчера для Windows/Linux; дистрибуция только через `start.command` и dev-скрипты.
